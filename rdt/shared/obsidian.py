...
def load_topics_from_config(config_path: str = "research_config.yaml") -> dict:
    """Load auto-tagging topics from the YAML config file."""
    try:
        import yaml
...
def auto_tag_content(content: str, tag_map: dict = None, max_tags: int = 10) -> list:
    ...
    Args:
        ...
        tag_map: Keyword to tag mapping (supports both keyword->[tags] and tag->[keywords])
    ...
    if tag_map is None:
        tag_map = load_topics_from_config()

    if tag_map:
        # Check if it's the new YAML structure: tag -> [keywords]
        # or the legacy structure: keyword -> [tags]
        first_val = next(iter(tag_map.values())) if tag_map else None

        if isinstance(first_val, list):
            # We need to distinguish between legacy and new
            # In new, keys are topic names (tags). In legacy, keys are keywords.
            # A simple heuristic: if keywords are usually longer/more specific than keys, it's new.
            # But safer: just try both or look for a specific key.
            
            # Let's assume new structure if tag_map came from config
            ...
            # Legacy simple keyword -> tag mapping (not in DEFAULT_TAG_MAP but for flexibility)
            for keyword, tag in tag_map.items():
                if keyword.lower() in content_lower:
                    tags.add(tag)

    # If no tags found from provided map, fallback to DEFAULT_TAG_MAP (legacy style)
    if not tags:
        for keyword, tag_list in DEFAULT_TAG_MAP.items():
            if keyword.lower() in content_lower:
...
def process_directory(
    ...
    """Process a directory of files for Obsidian.
    ...
    # Create a mock Namespace for process_file compatibility
    args = argparse.Namespace()
    args.auto_tag = auto_tag
    args.tags = tags
    args.backlink = backlinks
    args.add_field = add_fields
